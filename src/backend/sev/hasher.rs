...
use super::snp::launch::{IdBlock, PageInfo};
...
impl TryFrom<Hasher> for Vec<u8> {
    ...
    fn try_from(mut hasher: Hasher) -> anyhow::Result<Self> {
        ...
        let id_block = IdBlock {
            launch_digest: hasher.digest,
            family_id: hasher.config.parameters.family_id,
            image_id: hasher.config.parameters.image_id,
            guest_svn: hasher.config.parameters.guest_svn,
            policy: hasher.config.parameters.policy,
            ..Default::default()
        };
